importar numpy como np # Python matrix manipulación lib
class Simplex():
    ...
    def solve(self):
        ...
        self._b = np.array([-2,1],dtype=float)
                 self._A = np.array ([[1, -1,1]]) # Coeficiente de restricción de igualdad self._A, vector de columna dimensional 3x1
                 self._b = np.array ([2]) # Coeficiente de restricción de ecuación self._b, valor 1x1
    ...
    def InitializeSimplex(self, A, b):
                 ...
                 b_min, min_pos = (np.min (b), np.argmin (b)) # obtiene el bi mínimo
...
from scipy import optimize as op
 # Dar el rango de valores de la variable
x1=(0,None)
x2=(0,None)
x3=(0,None)
 c = np.array ([2,1,1]) # Coeficiente de función objetivo, vector de columna 3x1
 A_ub = np.array ([[0,2, -1], [0,1, -1]]) # Coeficiente de restricción de desigualdad A, matriz de 2x3 dimensiones
 B_ub = np.array ([- 2,1]) # Coeficiente de restricción de igualdad B, vector de columna dimensional 2x1
 A_eq = np.array ([[1, -1,1]]) # Coeficiente de restricción de igualdad Aeq, vector de columna dimensional 3x1
 B_eq = np.array ([2]) # coeficiente de restricción de ecuación beq, valor 1x1
 res = op.linprog (c, A_ub, B_ub, A_eq, B_eq, limits = (x1, x2, x3)) # Función de llamada para resolver
